...
| `RemoteError: Error submitting request to model deployment.` | The dispatch to the actor failed with something that isn't an eviction (`replica.py:296-299`) | [Stuck in QUEUED](#stuck-in-queued) |
| `RemoteError: Your request payload could not be read (...)` | The blob didn't deserialize — a `PayloadError` sentence, not a traceback | [A sentence instead of a traceback](#a-sentence-instead-of-a-traceback) |
| `RemoteError: The model architecture on this server doesn't match...` | An `ArchitectureMismatchError` — the server's model tree has no module at some `Module:<path>` | [A sentence instead of a traceback](#a-sentence-instead-of-a-traceback) |
| `RemoteError: Could not deploy this model. <reason>` | The controller refused and said why — a bad repo id, a gated repo, or no room | [model_key provisions, then errors](#a-model_key-that-provisions-and-then-errors) |
...
|---|---|---|---|---|
| 401 | `Missing or invalid API key. Please visit https://login.ndif.us/ ...` | `auth.py:98-102` | Auth is enabled (`NDIF_POSTGRES_URL` set) and no `ndif-api-key` header arrived | Set `CONFIG.API.APIKEY` / `NDIF_API_KEY` on the client |
| 400 | `Invalid API key format: '<key>'. API keys must be in the format: xxxxxxxx-...` | `auth.py:109-114` | The key isn't parseable as a UUID — usually a truncated paste or a placeholder | Re-copy the key |
...
| 422 | pydantic error list | `auth.py:162-165` | The `data` form field isn't JSON, or doesn't validate as a `BackendRequestModel`. Almost always a client/server schema mismatch | Upgrade nnsight; see [Version rejection](#version-rejection) |
| 503 | `Service temporarily unavailable: compute backend is reconnecting. Please try again in a few minutes.` | `app.py:114-119` | The `ray:connected` Redis flag is absent — the dispatcher is looping in `connect()` | Operator problem: [Troubleshooting](../operating/troubleshooting.md) |
...
| 500 | `Internal server error.` | `app.py:88-103` | Anything unhandled in ingress — a Redis write failure, a provider error | The real traceback is only in the API log (`just logs api`, logger `ndif.api`) |

Two follow-ups worth knowing:

- **401 vs 403 tells you whether auth is even on.** With `NDIF_POSTGRES_URL`
  unset, `verify_api_key` returns `None` immediately (`auth.py:93-95`) and *no* key
...
> (`queue/dispatcher.py:105`, `:129`), so if the dispatcher process *dies* the flag
...
`nnsight-version` / `python-version` headers (`versioning.py:31`, `:52`; the
gate itself is `validate_client_versions`, `versioning.py:77-87`). All rejections
are 400 and reach the user as `RemoteError`:

| Message | Meaning |
|---|---|
| ``Client nnsight version was not provided. This usually means an outdated nnsight; please `pip install --upgrade nnsight` and retry.`` | The header was missing. Old clients don't send it (`versioning.py:34-37`) |
| `Malformed nnsight version '<v>'.` | Unparseable by `packaging.version.Version` — a source checkout with no installed distribution reports `""` (`versioning.py:42`) |
| ``Client nnsight version X is below the minimum supported Y. Please `pip install --upgrade nnsight`.`` | Below the floor (`versioning.py:44-49`) — note the backticks are in the server's own message |
| `Client python version was not provided. ...` | Same, for the `python-version` header (`versioning.py:58-62`) |
| `Malformed python version '<v>'.` | Unparseable after major.minor is taken (`versioning.py:68`) |
| `Client python version 3.9 is below the minimum supported 3.10. Please update python.` | Only major.minor is compared (`versioning.py:70-74`) |
...
> **Gotcha:** both minimums are read **once at import** (`versioning.py:23-24`)
> and an empty string counts as unset (`or None`). Changing either needs an API
> restart.
...
- **Trusted (in-process).** `BaseModelDeployment.format_error` (`base.py:545`)
  runs nnsight's `clean_traceback` to drop nnsight plumbing, then
  `filter_traceback` to drop the actor's own wrapper frames (`base.py:558-560`).
- **Untrusted (sandboxed).** The runner formats the traceback in its own process
  ...
  passes it through verbatim (`SandboxHost.format_error`, `sandbox/model.py:161-166`).
...
| Description | Meaning |
|---|---|
| `Your job exceeded the execution timeout of {n}s.` | The `asyncio.wait` race lost against `execution_timeout` (`base.py:355-362`, message at `:399-403`). **There is no default**: `NDIF_DEFAULT_EXECUTION_TIMEOUT_SECONDS` is unset unless an operator sets it, so an unconfigured deployment never emits this |
| `Your job was cancelled or preempted by the server.` | The kill switch fired — `ndif kill`, or an operator cancel (`base.py:392-394`). A *preempt* (HOT→WARM under a running request) does **not** produce this: it raises `CachedActorError` instead and the request is silently re-queued (`base.py:383-386`) |
...
An `ERROR` whose `description` is one prose paragraph with no frames in it is a
**rejected request**, not a failed one: the server could not read or resolve the
payload, so the user's code never ran. Both come from
`BackendRequestModel.deserialize` (`common/schema/request.py:74-98`), which
classifies every deserialize failure into a `RequestError` subclass, and both
arrive identically on the trusted and sandboxed paths.

This is the triage rule that makes the two cases distinguishable at a glance:
**a traceback means the caller's code failed; a sentence means we rejected the
request** (`common/errors.py:19-28`).

| Message begins | Class | What to tell the user |
|---|---|---|
| `Your request payload could not be read (` | `PayloadError` (`errors.py:31`) | Re-send it — most of these are transit damage. If it repeats, it is a client/server nnsight mismatch or a package the block imports that the server doesn't have; compare with `ndif env` against `ndif env --local` |
| `The model architecture on this server doesn't match the one your code was traced against` | `ArchitectureMismatchError` (`errors.py:50`) | The message already carries the fix: run `from nnsight import ndif; print(ndif.compare())` and align whatever it flags, almost always `transformers` |

Neither is fatal to the replica — the request is unusable, the actor is fine
(`base.py:554-556` returns `fatal=False` for any `RequestError`). The
architecture case names the *first* path where the two module trees diverge, not
the layer the user was reaching for, because the block references the whole envoy
tree; don't read the named module as the one they got wrong.
...
| `PROVISIONING`/`DEPLOYING` for many minutes | `Replica.wait` polls `__ray_ready__` **with no timeout** (`queue/replica.py:148`) | `ndif status` — is the actor `DEPLOYING` or absent? |
| `QUEUED` twice, no error in between | A replica was evicted mid-flight and the request was silently pushed back to the **front** of the queue (`replica.py:273-280`) | expected behavior, not a bug |
...
Those strings come from `Processor.start` (`processor.py:190-192`, `:205-208`,
`:210-213`);
...
`Could not deploy this model. <reason>` (`processor.py:244-252`) — a mistyped
`repo_id`, a gated repo, or `CANT_ACCOMMODATE` on a full cluster. Only a
`DeploymentError` is passed through this way; the reason is truncated to
`_MAX_REASON_CHARS` with an ellipsis if it is long. These are the actionable cases, and
...
| `No GPU nodes available.` | `Cluster.nodes` is empty: the ray container has no GPU resource (`cluster.py:247`) |
