...
1. **A deployment is a set of detached Ray actors, one per replica.** Not a Ray
   ...
   (`.../cluster/deployment.py:210`). The name is
...
(`.../modeling/base.py:237`). It costs CPU RAM and process slots, but bringing it
...
> (`controller.py:516`). A model with nothing but `WARM` replicas looks
...
`ModelEvaluator.__call__` (`.../cluster/evaluator.py:187`) asks nnsight to describe
the checkpoint — `Remotable.describe_checkpoint(model_key, dtype,
trust_remote_code=...)` (`evaluator.py:155`), which reads the parameter count off
the checkpoint's metadata rather than building the architecture — takes
`described.size_bytes` as the base, then pads:
...
concrete value before evaluating (`controller.py:177`): the number that places
the replica must be the number the actor loads with.

## Placement

For each model in a deploy call (biggest first, `cluster.py:204`), for each
...
the actor's `trust_remote_code` (`controller.py:450`) *and* is passed to the
evaluator, so the size estimate and the real load agree even when repo code
changes the architecture (`cluster.py:183`). The queue sets it from the request
...
(`node.py:390`). Two rules gate what counts as evictable
(`node.py:327`):
...
(`node.py:257`) releases its GPU reservation, then — if the actor class says it
can be parked at all (`Deployment.cacheable`, read off the class's `CACHEABLE`,
`deployment.py:83`) — tries to make room in the node's CPU cache, greedily
dropping the smallest `WARM` entries if needed (`find_cache_evictions`,
`node.py:233`). If room exists, the replica is demoted to `WARM` **keeping its
`replica_id`**, so the actor name stays stable across the transition; if not, or
if the class isn't cacheable, it is removed outright and the actor is killed.
...
cancels the running request in `to_cache()` (`modeling/base.py:250`, before the
...
(`build`, `controller.py:339`; `apply`, `controller.py:388`):
...
request back at the front of the line, re-provision if work remains (`EVICTED_ERRORS`, `.../queue/replica.py:52`;
the handler at `replica.py:270`). An out-of-band deploy or evict (CLI, dashboard)
...
> RAM** budgeted for the WARM cache, not GPU memory. It is the wrong lever for a
> GPU OOM — nothing it scales reserves a byte of VRAM.

## Pinning and boot-time deployments

`NDIF_DEPLOYMENTS` is a `|`-separated list of model keys the controller deploys
**pinned** at startup (`controller.py:107`). Pinning is also available per deploy
...
Replicas the *queue* provisions are never pinned — `Replica.provision` calls
`controller.scale(model_key, 1, DeploymentConfig(trusted=processor.trusted))`
(`.../queue/replica.py:120`), and `DeploymentConfig.pinned` defaults to `False`
(`common/schema/controller.py:30`), so autoscaled capacity is always reclaimable.
