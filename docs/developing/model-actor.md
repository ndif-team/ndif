...
`BaseModelDeployment` (`src/ndif/services/ray/deployments/modeling/base.py:105`) is
...
`Deployment.create` (`controller/cluster/deployment.py:187`) resolves the actor
class and does (`:210`)
...
`deployment_args` is a `BaseModelDeploymentArgs` (`base.py:83`).

| Field | Default | Where it comes from |
|---|---|---|
| `model_key` | required | The nnsight model key (`"{import.path}:{json}"`) this deployment serves |
| `execution_timeout` | `None` | `DeploymentConfig.execution_timeout_seconds`, else `NDIF_DEFAULT_EXECUTION_TIMEOUT_SECONDS`, **unset by default**, so no cap at all (`controller.py:441`, `:776`) |
| `gpu_mem_bytes_by_id` | `{}` | Injected just before create from the placement the cluster chose (`deployment.py:190`) |
| `dtype` | `"bfloat16"` | `DeploymentConfig.dtype`, else `NDIF_DEFAULT_DTYPE` (`controller.py:449`, `:791`) |
| `trust_remote_code` | `False` | `DeploymentConfig.trusted`, from the `trusted` flag of the request that triggered the deployment (`controller.py:450`, `cluster.py:183`) |

Only these five fields cross the boundary — a constructor parameter that isn't a
field here can never be set by the controller. `__init__` (`base.py:118`) binds the
...
`__init__` connects Loki and Influx first (`base.py:132`) — each Ray actor is its own
...
(`_provider_runtime_env`, `deployment.py:16`, called at `:200`) along with `NDIF_SERVICE=model`,
...
(`Deployment.name`, `deployment.py:123`), which is what the queue looks up
(`get_model_actor_handle`, `common/providers/ray.py:226`).

## Loading the model

`load_from_disk` (`base.py:173`) runs in the constructor: the actor isn't `__ray_ready__` until the weights are on the GPUs.
...
- After a `torch.cuda.synchronize()` (accelerate's copies are async),
  ...
  emitted with `load_type="initial"` (`base.py:228`).
...
> wrong. Hence the controller pins `config.dtype` before evaluating (`controller.py:179`).
...
`_remoteable_set_env` (`base.py:350`) just before execution. nnsight's
...
The controller moves a replica between levels through two `Deployment` methods (`cache`, `deployment.py:172`; `from_cache`, `:179`):

| Call | Method | What happens |
|---|---|---|
| HOT→WARM | `to_cache()` (`base.py:237`) | `cancel()` any in-flight run, `_module.to("cpu")`, `reset_process_limits`, `synchronize` + `gc.collect` + `empty_cache`, `self.cached = True` |
| WARM→HOT | `from_cache(gpus)` (`base.py:262`) | Re-apply limits for the (possibly different) GPUs, compute a balanced `device_map`, `remove_accelerate_hooks` then `dispatch_model`, verify, `self.cached = False`, emit `ModelLoadTimeMetric(load_type="from_cache")` |
...
is true, `run()` raises `CachedActorError` *before* its try block (`base.py:315`) so
...
`BackendRequestModel.trusted` (`common/schema/request.py:56`, default `False`) is
...
- **Trusted** → `BaseModelDeployment.execute` (`base.py:473`), in the actor process,
  in the same address space as the weights. That is the path documented below.
- **Untrusted** → `SandboxModelDeployment.execute` (`sandbox/model.py:207`) ships the
...
(`Replica.dispatch`, `replica.py:207`; the Ray call is at `:228`). The request carries `payload` (the serialized
...
sequenceDiagram
    ...
    R->>R: prepare_result — compress, meter
    alt blob over the socket cap, or no session_id
        R->>S: put request-id.pt, then presign a GET url
    end
    R->>R: finally restore snapshots, cleanup
    R->>C: respond COMPLETED with the blob or the url
```

### The race

Execution runs on a worker thread so the event loop can race it (`base.py:354`):
...
(`base.py:541`) calls `kill_thread` (`util.py:69`), injecting `SystemExit` via
...
`execute` (`base.py:473`) is the hook a subclass replaces to run the block elsewhere:

```python
self.execution_ident = threading.current_thread().ident
tracer, deserialize_ms = prepare_traced_block(
    self.model, request.payload, request.compress,
...
The two halves live in `deployments/modeling/nns.py`, split so the
tensor-parallel actor can reuse them — its shards prove they can *build* a block
before rank 0 commits to a forward pass.

- `prepare_traced_block` (`nns.py:139`) recomputes
  `model._remoteable_persistent_objects()` *after* the env swap, so a PEFT rebind
  is reflected in the `Module:<path>` ids the payload resolves against, then calls
  `BackendRequestModel.deserialize` — which recompiles the block from its shipped
  **source**, not bytecode, registers it in `linecache` so a traceback can show
  the user's line even though their file doesn't exist here, and classifies every
  failure into a caller-facing `RequestError`.
- `commit()` (`base.py:504`) is the seam between *can this request run* and
  *running it* — empty in the base actor, where the tensor-parallel one releases
  its other ranks.
- `execute_traced_block` (`nns.py:161`) seeds the block, enters the
  `request_dtype` autocast region (`nns.py:75` — thread-local, which is why it
  lives here and not in `run()`), and brackets `tracer.execute` in `inc()`/`dec()`
  the way the client's `Tracer.__exit__` does, so `nnsight.save()` records into
  this thread's save set. Saves are matched **by identity** against frame
  locals — an unbound `.save()` has no name and never appears in the result.
- `cpu_pickle_module()` (`util.py:287`) is a `pickle` stand-in whose `Pickler`
...
(`common/schema/request.py:161`): a blocking request (has `session_id`) is published
...
`GET /response/{id}`. The actor emits `RUNNING` on entry (`base.py:317`), zero or
more `LOG`s, and exactly one `COMPLETED`/`ERROR`. `LOG` comes from `execution_scope`
(`base.py:516`), which redirects `sys.stdout` to a `LogStream` (`util.py:16`)
...
An exception out of the raced block is formatted by `format_error` (`base.py:545`)
...
*fatal* flag — the two CUDA messages at `base.py:72` ("device-side assert triggered",
"an illegal memory access was encountered") poison the process's CUDA context
permanently, so `finally` calls `restart()` (`base.py:636`), i.e. `ray.kill(
current_actor, no_restart=False)`, and `max_restarts=-1` brings the replica back.

### The finally block, and the upload

`finally` (`base.py:446`) exits the `block_scope()` (`nns.py:37`) entered at
`base.py:327`, restoring `linecache.cache` and nnsight's process-global
`SOURCES`/`BLOCKS` memos from the snapshots taken on entry — they're keyed by
...
`restart()` on a fatal error, else `cleanup()` (`base.py:646`): clear the kill switch,
...
`report()` (`base.py:563`) emits a structured `event()` log for non-completed
...
| `GPUMemMetric` | `gpu_mem` | after a successful run (`base.py:410`) | per device `baseline_bytes`, `peak_bytes`, `extra_bytes` |
...
resolves (`base.py:339`, `:367`), so **`exec_ms` spans the whole worker-thread body
...
- **Readiness.** There is no health RPC; the controller and the CLI wait on
  `actor.__ray_ready__.remote()` (`controller.py:457`, `replica.py:166`), which
...
- **Eviction / shutdown.** `Deployment.delete()` is `ray.kill(actor,
  no_restart=True)` (`deployment.py:159`). There is no graceful-shutdown hook —
  nothing calls `close()`/`__del__`, so a subclass holding OS resources loses them
  with the process.

## The template hooks

`ModelActor` (`base.py:702`, decorated `@ray.remote(num_cpus=1, max_restarts=-1)`) is
...
(`sandbox/model.py:191`) overrides exactly those five, plus `restart` and `close`.
...
`ModelActor` (`ControllerDeploymentArgs.model_import_path`, `controller.py:741`). The
compose stack sets `NDIF_DEFAULT_MODEL_ACTOR_CLASS` to
`ndif.services.ray.sandbox.model.SandboxModelActor` (`docker/docker-compose.yml:252`),
