...
picked up a `model_key` is an orphan. Ray retains a `DEAD` record for every actor it has ever run, so
every evict and every restart leaves one behind. Unfiltered they accumulate monotonically and, on a
cluster that swaps models routinely, come to outnumber the real entries — 9 orphans against 4 real
deployments after a day of testing — breaking consumers that read `deployments` and assume a
`repo_id`. A dead actor the controller *does* still track survives the
