...
| `ndif start` printed ✓ but `/connected` never leaves `reconnecting` | The service died right after spawning (a ✓ now waits 2 s, but a slower death still gets one) | `ndif info` — is `ray` `stopped`? `$NDIF_HOME/logs/ray.log` | Read the log; `validate_socket_filename failed: AF_UNIX path length cannot exceed 107 bytes` means `NDIF_RAY_TEMP_DIR` is too long |
| `ndif stop` said stopped but `gcs_server` / `raylet` are still running | An older `ndif stop` only killed the `start.sh` process group; `ray start` daemonises | `ps -ef \| grep gcs_server` | `ndif stop` now runs `ray stop --force` for the ray service; on older installs run it yourself |
