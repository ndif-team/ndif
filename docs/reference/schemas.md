...
| `priority` | `bool` | `False` | Jump the queue. Stamped at ingress from the key's `priority` user_tag (`Identity.priority`, `auth.py:80`) and consumed in `Dispatcher.dispatch`, which enqueues with `prepend=request.priority` (`dispatcher.py:148`) — `Processor.enqueue` then puts it at the head of the deque and wakes an idle replica (`processor.py:109`). That is the whole feature: one position jump at dispatch, no priority classes or aging. |
