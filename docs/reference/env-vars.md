...
`CMD ["start", "--foreground"]` (`docker/Dockerfile:138-139`) — so **the CLI
...
| `NDIF_VERSION` | **nothing** — it is a `docker build` ARG (`docker/Dockerfile:112`), not an environment variable any process reads | Passed by the publish workflow so the version lands on the `org.opencontainers.image.version` label; `VCS_REF` and `BUILD_DATE` are its siblings (`Dockerfile:113-114`). It is the one `NDIF_*` name in the repo that is not a runtime knob. The *resolved* package versions go to `/etc/ndif/build.json` instead, written at build time by `ndif version --write` (`Dockerfile:108`); `ndif version` in a running container re-reads them from the live install rather than from that file. |
