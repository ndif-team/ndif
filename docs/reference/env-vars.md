...
Containers run `ndif start --foreground` — `ENTRYPOINT ["ndif"]` plus
`CMD ["start", "--foreground"]` (`docker/Dockerfile:141-142`) — so **the CLI
defaults apply inside compose too**. For
...
Influx settings into the actor's Ray `runtime_env` (`cluster/deployment.py:16-22,
:195-215`).
...
| `NDIF_API_URL` | `http://localhost:8001` | `cli/config.py:26`, `cli/commands/env.py:46`, `cli/commands/doctor.py:97`, `dashboard/start.sh:64` | Where *clients of the API* look for it: the CLI's `env`/`info`/`doctor` checks and the dashboard's `/api/status` proxy. Does not affect what the API binds. |
...
| `NDIF_API_KEY` | unset | `src/ndif/services/dashboard/jobs/monitor.py:404`, `dashboard/start.sh:65` | A *client* key, used only by the dashboard's monitor cron so its synthetic model traces can authenticate. Nothing in the API or CLI reads it. |
...
> (`docker/docker-compose.yml:237`). The default `localhost:6379` inside that
...
| Variable | Default | Read by | Effect |
|---|---|---|---|
| `NDIF_RAY_ADDRESS` | `ray://localhost:10001` | `src/ndif/common/providers/ray.py:55`, `cli/config.py:28` | The `ray://` **client** address the API, dashboard and CLI connect with. Says nothing about head-vs-worker. A URL with no port logs a warning and falls back to 6379 (`providers/ray.py:73-80`). |
...
| Variable | Default | Read by | Effect |
|---|---|---|---|
| `NDIF_DEPLOYMENTS` | `""` | `src/ndif/services/ray/deployments/controller/controller.py:735` | Model keys to deploy at controller start, separated by a pipe character. Note that splitting the empty default yields `[""]`, not `[]`. |
| `NDIF_CONTROLLER_SYNC_INTERVAL_S` | `30` | `controller.py:164` | Seconds between `Cluster.update_nodes` reconcile passes. |
| `NDIF_DEFAULT_EXECUTION_TIMEOUT_SECONDS` | unset (no cap) | `controller.py:773-778` | Per-request execution cap handed to each model actor. Unset means a request runs until it finishes, so a long job is never cut off on a single-user deployment. Set it on anything shared: with no cap a runaway block holds its replica indefinitely, and `NDIF_AUTOSCALING_MAX_REPLICAS` (3) of those take the model out. A per-model `execution_timeout_seconds` overrides it either way. |
| `NDIF_MODEL_IMPORT_PATH` | falls back to `NDIF_DEFAULT_MODEL_ACTOR_CLASS`, then `ndif.services.ray.deployments.modeling.base.ModelActor` | `controller.py:741-747` | Dotted import path of the model actor class the controller builds each deployment from. Resolution order: `NDIF_MODEL_IMPORT_PATH` → `NDIF_DEFAULT_MODEL_ACTOR_CLASS` → the base `ModelActor`. A per-deployment `DeploymentConfig.actor_class` still overrides it (`_ControllerActor.__init__` stores it as `default_model_actor_class`). |
| `NDIF_CONTROLLER_IMPORT_PATH` | `ndif.services.ray.deployments.controller.controller.ControllerActor` | `controller.py:764-767` | Dotted import path of the controller actor class `app()` launches (`_import_from_path`, `controller.py:29`, called at `controller.py:806`). Lets an operator swap in a `ControllerActor` subclass without editing the module. |
| `NDIF_TP_MODEL_ACTOR_CLASS` | **unset — tensor parallelism off** | `controller.py:759-761` | The actor class a *tensor-parallel* replica gets. Unset is not a fallback to the built-in one: it disables the feature outright, so no sharding degree is worked out, no GPU count is rounded up to one a model splits into evenly, and a per-model `max_tp` does nothing. Set it to `ndif.services.ray.tp.model.TPModelActor` to enable it, to `ndif.services.ray.tp.model.SandboxedTPModelActor` if the cluster takes untrusted traffic (same trusted path, untrusted blocks run in one runner the group hosts), or to a subclass of either. Opt-in because a sharded replica cannot be cached and needs transformers >= 5.15. |
| `NDIF_DEFAULT_MODEL_ACTOR_CLASS` | `ndif.services.ray.deployments.modeling.base.ModelActor` | `controller.py:743-745` | The **fallback** for `NDIF_MODEL_IMPORT_PATH` rather than an independent knob. The dev compose sets it to `ndif.services.ray.sandbox.model.SandboxModelActor` (`docker/docker-compose.yml:252`), which — absent `NDIF_MODEL_IMPORT_PATH` — is what wins, so **the compose stack runs sandboxed by default and a bare controller does not**. |
| `NDIF_MINIMUM_DEPLOYMENT_TIME_SECONDS` | `3600` | `controller.py:779-781` | How long a replica is protected from eviction after it comes up. |
| `NDIF_MODEL_CACHE_PERCENTAGE` | `0.9` | `controller.py:782-784` | **Scales CPU RAM, not GPU memory.** It is the WARM-cache budget: `cluster.py:120-122` multiplies the node's advertised `cpu_memory_bytes` by it. `resources.py:18-22` advertises *total* RAM deliberately (so the budget is stable regardless of what else is resident), and this scales it down. `cuda_memory_bytes` is untouched by this variable. |
| `NDIF_DEFAULT_PADDING_FACTOR` | `0.15` | `controller.py:785-787` | Multiplicative slack in the memory estimate used to place a replica. |
...
| Variable | Default | Read by | Effect |
|---|---|---|---|
| `NDIF_POSTGRES_URL` | `""` | `src/ndif/common/providers/postgres.py:57` | **The auth master switch — and, indirectly, the isolation switch.** See the callout below. Set → keys are checked against the `keys` table, failing *closed* (503) if the DB is unreachable. If it's set and `asyncpg` isn't installed the API raises rather than silently disabling auth (`postgres.py:92-96`). |
| `NDIF_POSTGRES_POOL_MIN` | `1` | `postgres.py:60` | Connections opened up front on first use. |
| `NDIF_POSTGRES_POOL_MAX` | `10` | `postgres.py:61` | Pool ceiling under concurrency. |
| `NDIF_POSTGRES_COMMAND_TIMEOUT_S` | `10.0` | `postgres.py:64` | Per-statement timeout, so a wedged DB surfaces as an error instead of hanging requests. |
...
> `validate_request` stamps `request.trusted = True` on any request that didn't
> set it itself (`src/ndif/services/api/auth.py:180-184`); an explicit
> `trusted: false` from the client is honored. Two things follow from that flag:
>
...
>   subprocess (`sandbox/model.py:218-220`; same branch in `execution_scope`,
>   `model.py:157`). User code executes inside the model actor, next to the weights.
> - **Models load with `trust_remote_code`.** The flag rides from
>   `request.trusted` through `Processor.ensure_started`
>   (`queue/processor.py:119, 164-165`) into the `DeploymentConfig` the replica
>   deploys with (`queue/replica.py:121`) and on to the HF load
>   (`controller/cluster/cluster.py:183, :225, :296`).
>
> With Postgres configured, `trusted` instead comes from the key's `trusted`
> user_tag (`auth.py:74-77, 178`), so ordinary users are sandboxed. **Self-hosting
> NDIF anywhere but a trusted LAN means setting `NDIF_POSTGRES_URL`.** The dev
> compose leaves it commented out (`docker-compose.yml:165`), so the dev stack is
...
| `NDIF_INFLUX_ENABLED` | `true` | `influx.py:70` | Hard off-switch for metric writes, independent of the URL. `_boolish` (`influx.py:56-57`). |
...
| Variable | Default | Read by | Effect |
|---|---|---|---|
| `NDIF_LOKI_URL` | `""` | `src/ndif/common/providers/loki.py:139` | **The log-shipping switch.** Empty → `logging_loki` is never imported and nothing ships; the readable console handler is configured either way (`loki.py:160-184`). Set it to a push endpoint (`http://loki:3100/loki/api/v1/push`) to enable. If it's set but the package is missing, you get a warning and console-only logs (`loki.py:185-191`). |
...
| Variable | Default | Read by | Effect |
|---|---|---|---|
| `NDIF_DASHBOARD_PORT` | `8081` | `src/ndif/services/dashboard/start.sh:36` | Port uvicorn binds, on `0.0.0.0` (`dashboard/start.sh:95-96`). |
...
| `NDIF_DASHBOARD_DEV_MODE` | `false` | `backend/config.py:39` | **`true` bypasses auth on every route.** The dev compose sets it (`docker-compose.yml:216`); remove it for any real deployment. |
| `NDIF_DASHBOARD_DATA_DIR` | `~/ndif_dashboard` | `backend/config.py:41`, `dashboard/jobs/util.py:16`, `dashboard/start.sh:37` | Root for `logs/`, `schedule.json`, `.reconcile.state.json`, `config.json`, `cache/`. Compose points it at the `dashboard_data` volume. |
| `NDIF_DASHBOARD_FRONTEND_DIST` | `<package>/frontend/dist` | `backend/config.py:42-44` | Built Vue SPA directory to serve. |
| `NDIF_DASHBOARD_MONITOR_URL` | `http://localhost:8001` | `dashboard/start.sh:76` | What the monitor cron probes. In prod point it at the *public* URL so the probe exercises DNS, TLS and the load balancer. |
| `NDIF_DASHBOARD_MONITOR_CRON` | `*/10 * * * *` | `dashboard/start.sh:76` | Crontab expression for the uptime monitor. |
| `NDIF_DASHBOARD_RECONCILE_CRON` | `*/2 * * * *` | `dashboard/start.sh:77` | Crontab expression for the schedule reconciler. |
| `NDIF_DASHBOARD_REPORT_CRON` | `0 0 * * *` | `dashboard/start.sh:78` | Schedule for the daily usage/uptime digest posted to Discord (`jobs/report.py`). |
| `NDIF_DASHBOARD_REPORT_WINDOW_HOURS` | `24` | `dashboard/start.sh:78`, `jobs/report.py:77` | How far back that digest looks. |

`cron` strips the environment, so `start.sh:60-73` writes an explicit variable
...
| `NDIF_INFLUX_ENABLED=false` / no `NDIF_INFLUX_TOKEN` | No metrics. `enabled` short-circuits `connect` (`influx.py:110-112`); a bad token means writes are rejected and dropped. Either way the service keeps running. |
...
from these same variables (`src/ndif/cli/commands/doctor.py:94-99`) and also
reports the resolved ndif / nnsight / torch (+CUDA) / transformers / ray versions
(`doctor.py:38-52`, shared with `ndif version`).
...
| `RAY_EXPERIMENTAL_NOSET_CUDA_VISIBLE_DEVICES` | set to `1` in `cluster/deployment.py:195` | Stops Ray from masking GPUs from a model actor; GPU targeting is done through `max_memory` in the actor instead. |
| `PYTORCH_CUDA_ALLOC_CONF` | set to `expandable_segments:True` in `cluster/deployment.py:197` | Reduces CUDA memory fragmentation in the actor process. |
| `MINIO_ROOT_USER` / `MINIO_ROOT_PASSWORD` | written by `src/ndif/cli/service.py:48-49`; set in `docker-compose.yml:130-131` | MinIO reads its root credentials from these, not from `NDIF_*`; the CLI mirrors the object-store keys into them. |
| `HF_TOKEN` | host passthrough on the `ray` service (`docker/docker-compose.yml:256`), and passed through in `dashboard/start.sh:73-74` | Hugging Face auth. Compose forwards the host's `HF_TOKEN` into the `ray` service so gated repos (Llama, etc.) load, and the dashboard forwards it into the reconcile cron for the same reason. Read by `transformers` / `huggingface_hub`, not by NDIF code. Compose also bind-mounts the host HF cache (`${HOME}/.cache/huggingface` → `/root/.cache/huggingface`, `docker-compose.yml:266-267`); the image declares the same path as a `VOLUME` (`docker/Dockerfile:137`) so downloaded weights persist across container restarts. |
| `HF_HOME` | passed through in `dashboard/start.sh:73-74` | Hugging Face cache location. Read by `transformers` / `huggingface_hub`. The compose `ray` service leaves it at the default `/root/.cache/huggingface`, which is the bind-mount target above. |
| `PYTHONPATH` | extended in `src/ndif/services/ray/sandbox/host.py:174-176` | The sandbox runner is spawned with the package root prepended so it can import `ndif.services.ray.sandbox.runner`. |
| `NDIF_VERSION` | **nothing** — it is a `docker build` ARG (`docker/Dockerfile:115`), not an environment variable any process reads | Passed by the publish workflow so the version lands on the `org.opencontainers.image.version` label; `VCS_REF` and `BUILD_DATE` are its siblings (`Dockerfile:116-117`). It is the one `NDIF_*` name in the repo that is not a runtime knob. The *resolved* package versions go to `/etc/ndif/build.json` instead, written at build time by `ndif version --write` (`Dockerfile:111`); `ndif version` in a running container re-reads them from the live install rather than from that file. |
| `DOCKER_INFLUXDB_INIT_*`, `POSTGRES_*`, `GF_*` | `docker/docker-compose.yml:46-52, 106-110, 80-91` | Container-image bootstrap variables for InfluxDB, Postgres and Grafana. They must stay consistent with the `NDIF_INFLUX_*` and `NDIF_POSTGRES_URL` the services use. |
