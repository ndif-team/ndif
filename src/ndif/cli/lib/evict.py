"""Programmatic implementation of ``ndif evict`` (Ray controller only)."""
...
def evict(
    *,
    ...
    By default every HOT and WARM replica of each target is evicted; pass
    ``replica=<id>`` with a single-model selector to target one.

    Returns ``{"results": [{"model_key", "status", "replicas": [...]}]}``.
    """
    import ray
    ...
    if sum(modes) != 1:
        raise ValueError("Specify exactly one of: checkpoints, model_keys, evict_all")

    if replica is not None:
        if evict_all:
            raise ValueError("--replica cannot be combined with evict_all")
        if (checkpoints and len(checkpoints) > 1) or (model_keys and len(model_keys) > 1):
            raise ValueError("--replica targets a single replica — supply exactly one model")
    ...
    if evict_all:
        target_keys = sorted({
            d["model_key"] for d in get_current_deployments(level="HOT") if "model_key" in d
        })
        if not target_keys:
            return {"results": []}
    elif checkpoints:
    ...
    for mk in target_keys:
        ...
        for r in replicas:
            ...
            emit(on_message, f"      - [{r.replica_id}] {len(r.gpus)} GPU(s), {round(freed_gbs, 4)} GB")
            records.append({
    ...
    # Nudge any already-active dispatcher Processors to drop the evicted replicas.
    notify_reconcile(redis_url, target_keys)
