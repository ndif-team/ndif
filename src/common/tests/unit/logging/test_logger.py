...
@pytest.fixture
def log_record():
    return logging.LogRecord(name="test",
                             level=logging.INFO,
                             pathname="test.py",
                             lineno=123,
                             msg="hello",
...
def test_custom_json_formatter(log_record):
    formatter = CustomJSONFormatter("api",
                                    fmt="%(service_name)s "
                                    "%(hostname)s "
                                    "%(process_id)d "
                                    "%(thread_name)s "
...
def test_loki_handler_retries_then_succeeds(log_record):
    ...
    with patch("logging_loki.LokiHandler.emit", side_effect=emit_exceptions) as mock_emit, patch("time.sleep") as mock_sleep:
