...
class _ControllerActor:
    ...
    def _deploy(self, deployments: Union[MODEL_KEY, List[MODEL_KEY], Dict[MODEL_KEY, DeploymentConfig]]):
        ...
        self.logger.info(f"Deploying models: {[(key, cfg.dedicated, cfg.replicas) for key, cfg in configs.items()]}")
    ...
    def _remove_deployment_from_state(self, deployment: Deployment) -> None:
        ...
        if deployment.node_id and deployment.node_id in self.cluster.nodes:
            ...
            if model_map and deployment.replica_id in model_map:
                ...
                node.gpu_resources.release_memory(deployment.gpu_mem_bytes_by_id)
