    ...
    def status(self):
        ...
        for model_key, schedule in schedule.items():
            ...
            if application_name in status["deployments"]:
                ...
                self.cluster.gpu_resource_evaluator(model_key)

                repo_id = self.cluster.gpu_resource_evaluator.cache[model_key].config._name_or_path
                ...
                status["deployments"][application_name] = {
                    ...
                    "model_key": model_key,
                    "repo_id": repo_id,
                    "revision": self.cluster.gpu_resource_evaluator.cache[model_key].revision,
                    "config": self.cluster.gpu_resource_evaluator.cache[
                        model_key
                    ].config.to_json_string(),
                    "schedule": schedule,
                    "n_params": self.cluster.gpu_resource_evaluator.cache[model_key].n_params,
                }
