...
- Install dependencies (nnsight, requests, flask)
- Copy source files into `<INSTALL_DIR>/jobs/` and `<INSTALL_DIR>/dashboard/`
- Create `<INSTALL_DIR>/config.json` from `config.example.json` if missing
- Install/update the cron job

Re-run `run.sh` after making changes to deploy updates. The deployed monitor runs from the install dir, so repo/branch changes won't affect it.
...
<INSTALL_DIR>/
  ...
  jobs/
    monitor.py        Main monitor script (runs via cron)
...
<INSTALL_DIR>/../../miniconda3/envs/monitor/bin/python3 \
  <INSTALL_DIR>/dashboard/dashboard.py --log-dir <INSTALL_DIR>/logs
```
...
src/services/monitor/
  ...
  jobs/
    monitor.py            Unified monitor script
    util.py               Shared utilities (config, discord, log rotation)
  dashboard/
    dashboard.py          Flask backend (serves API + HTML)
