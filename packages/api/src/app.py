...
@app.post("/request")
async def request(raw_request: Request) -> BackendResponseModel:
    ...
    # process the request
    try:
        request: BackendRequestModel = BackendRequestModel.from_request(raw_request)
        ...
        try:
            ...
            logger.info(
                f"Sending request to queue: {os.environ.get('QUEUE_URL')}/queue"
            )
            ...
            logger.info(
                f"Request sent to queue successfully: {os.environ.get('QUEUE_URL')}/queue"
            )
    ...
    try:
        ...
        if e.response.status_code == 404:
            ...
            try:
                ...
                error_detail = str(e)
            raise HTTPException(
                status_code=500, detail=f"Failed to delete request: {error_detail}"
            )
        else:
