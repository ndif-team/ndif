...
def ready_to_be_parallelized(module: torch.nn.Module):
    ...
    return not isinstance(param, DTensor) and param.nonzero().numel() != 0
...
def parallelize_model(model: torch.nn.Module, model_name: str, tp_mesh):

    model_plans = model_name_to_plans[model_name]
